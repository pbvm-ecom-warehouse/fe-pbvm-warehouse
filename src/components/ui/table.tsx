...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-border/70 transition-colors hover:bg-accent/50 has-aria-expanded:bg-accent/50 data-[state=selected]:bg-accent",
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "px-3 py-2.5 align-middle whitespace-nowrap text-foreground/85 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
